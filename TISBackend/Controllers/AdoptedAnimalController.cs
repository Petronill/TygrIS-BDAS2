    ...
    public class AdoptedAnimalController : TISControllerWithInt
    {
        ...
        public IEnumerable<Animal> GetAll()
        {
            ...
            DataTable query = DatabaseController.Query($"SELECT t1.*, t2.*, t3.*, t4.*, t5.*, t6.*, t6.nazev AS nazev2 FROM {TABLE_NAME} t1 " +
                ...
                $"WHERE t1.{ID_ADOPTUJICI} IS NOT NULL");
            foreach (DataRow dr in query.Rows)
            {
        ...
        public IEnumerable<int> GetIds(int id)
        {
            ...
            if (IsAuthorized())
            {
                DataTable query = DatabaseController.Query($"SELECT {ID_NAME} FROM {TABLE_NAME} WHERE {ID_ADOPTUJICI} = :id", new OracleParameter("id", id));
                foreach (DataRow dr in query.Rows)
                {
                    list.Add(int.Parse(dr[ID_NAME].ToString()));
                }
